...
const VideoEditorExample = () => {
  ...
  const [videoLayers, setVideoLayers] = useState([
    ...
    { id: "main", start: 7500, end: 25000, type: "video" },
    { id: "outro", start: 24500, end: 30000, type: "video" },
  ]);
  ...
  const updateVideoLayer = useCallback(
    (layerId: string, newStart: number, newEnd: number) => {
      setVideoLayers((prev) =>
        prev.map((layer) =>
          layer.id === layerId
  ...
  return (
    <div className="space-y-4 p-6 bg-gray-50 rounded-lg">
      ...
      <Timeline.Root
        ...
        timelineBounds={30000}
        constraints={constraints}
        step={100}
        className=""
      >
        <Timeline.Content className="h-64">
